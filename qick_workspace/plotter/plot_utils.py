...
def hist(data, plot=True, span=None, ax=None, verbose=False, qubit=0):
    ...
    if plot:
        if ax is None:
            ...
            if plot_f:
                axs[0, 0].plot(
                    If, Qf, ".", label="f", color=GREEN, alpha=a, markersize=m
                )
        ...
        if plot_f:
            ax[0].plot(
                If_new, Qf_new, ".", label="f", color=GREEN, alpha=a, markersize=m
            )
        ...
        if "vhg" in data and "histg" in data:
            ...
            if (
                ...
                ax[1].plot(
                    data["vhg"],
                    gaussian(data["vhg"], 1, *data["paramsg"]),
                    "k",
                    linewidth=1,
                )
                ax[1].plot(
                    data["vhe"],
                    excited_func(
                        data["vhe"], data["vg"], data["ve"], data["sigma"], data["tm"]
                    ),
                    "k",
                    linewidth=1,
                )
...
def plot_final(xpts, data: np.ndarray, x_label: str, fitfunc, simfunc, return_ax=False):
    ...
    for i, measure in enumerate(measures):
        ...
        ax.plot(data["xpts"], data[measure], **marker_style)
        ax.set_xlabel(x_label)
        ax.set_ylabel(f"{measure} (ADC unit)")
        if f"fit_{measure}" in data:
            ax.plot(
                data["xpts"],
                simfunc(data["xpts"], *data[f"fit_{measure}"]),
            )
        ax.set_title(measure)

    ax_big = fig.add_subplot(gs[:, 2])
    ax_big.set_title(f"Best fit: {best_measure}")
    ax_big.plot(data["xpts"], data[best_measure], **marker_style)
    ax_big.plot(data["xpts"], simfunc(data["xpts"], *fit_params))
