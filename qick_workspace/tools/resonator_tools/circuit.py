...
class reflection_port(circlefit, save_load, plotting, calibration):
    ...
    def get_single_photon_limit(self, unit="dBm"):
        """
        Returns the power (in W or dBm) necessary
        ...
        P_watt = (2 * np.pi * hbar * fr * (k_c + k_i) ** 2) / (4 * k_c)

        if unit.lower() == "dbm":
            return Watt2dBm(P_watt)
        elif unit.lower() == "watt":
            return P_watt
        else:
            raise ValueError("unit must be 'dBm' or 'watt'")
